...
export const SalesProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // TOTAL ROW
  const totalRow = sales.find((x) => x.operatingUnit?.toLowerCase() === "total");

  // KPI VALUES
  const totalTurnoverCurrentFy = totalRow?.thisYear?.salesYtd || 0;

  const totalTurnoverPreviousFy = totalRow?.lastYear?.salesYtd || 0;

  const totalPendingOrders = totalRow?.thisYear?.pendingOrdersYtd || 0;
  ...
  const tableData = sales
    ...
    .map((item) => {
      ...
      return {
        ...
        fy2627SalesAsOn: item.thisYear.salesYtd,

        fy2627SalesCurrentMonth: item.thisYear.salesThisMonth,

        fy2526SalesAsOn: item.lastYear.salesYtd,

        fy2526SalesCurrentMonth: item.lastYear.salesThisMonth,

        trend,

        fy2627PendingAsOn: item.thisYear.pendingOrdersYtd,

        fy2627PendingCurrentMonth: item.thisYear.pendingThisMonth,

        fy2526PendingAsOn: item.lastYear.pendingOrdersYtd,

        fy2526PendingCurrentMonth: item.lastYear.pendingThisMonth,
  ...
  const rows: OperatingUnit[] = sales
    ...
    .map((item) => ({
      ...
      to_fy27_month: formatMoney(item.thisYear.salesThisMonth),
      to_fy26_date: formatMoney(item.lastYear.salesYtd),
      to_fy26_month: formatMoney(item.lastYear.salesThisMonth),
      trend: formatTrend(item.thisYear.salesYtd, item.lastYear.salesYtd),
