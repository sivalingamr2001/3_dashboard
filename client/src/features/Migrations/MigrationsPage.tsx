...
import { Play, RefreshCw, AlertTriangle, CheckCircle, Terminal, FileText } from 'lucide-react';
...
export const MigrationsPage = () => {
    ...
    const handleRunMigration = async () => {
        ...
        try {
            ...
            setMigrationResult({
                success: false,
                message: error?.response?.data?.errorMessage || error.message || "Migration process failed."
            });
        } finally {
            setLoading(false);
        }
    ...
    const handleFetchLogs = async () => {
        ...
        try {
            ...
            setLogLines([{ id: 'err', type: 'log', raw: 'Failed to access remote log streams.', level: 'ERR' }]);
        } finally {
            setLogLoading(false);
        }
    };
    ...
    return (
        <div className="p-6 space-y-6 max-w-5xl mx-auto font-sans antialiased">
            {/* Header Dashboard Metrics */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-5">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900">Database Migrations</h1>
                    <p className="text-sm text-muted-foreground mt-1">
                        Manual sync trigger and workspace console tracking for <code className="bg-slate-100 px-1 py-0.5 rounded text-xs">bin\logs\MigrationLogs</code>
                    </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <Button
                        ...
                        disabled={logLoading}
                        className="gap-2 text-slate-700 border-slate-200 shadow-sm"
                    >
                    ...
                    <Button
                        ...
                        disabled={loading}
                        className="gap-2 bg-blue-600 hover:bg-blue-700 shadow-sm text-white"
                    >
            ...
            {migrationResult && (
                <div className={`p-4 rounded-lg border flex items-start gap-3 shadow-xs transition-all ${migrationResult.success
                    ...
                    <div>
                        <h4 className="font-semibold text-sm">{migrationResult.success ? 'Execution Completed' : 'Pipeline Halt Error'}</h4>
                        <p className="text-sm opacity-90 font-mono mt-0.5">{migrationResult.message}</p>
                    </div>
                </div>
            )}
            ...
            <Card className="border-slate-200 shadow-sm overflow-hidden">
                <CardHeader className="bg-slate-50 border-b border-slate-100 py-4 px-6 flex flex-row items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <CardTitle className="text-sm font-semibold text-slate-700">Serilog Console Output</CardTitle>
                    </div>
                        ...
                        {logLines.length > 0 ? (
                            <div className="space-y-1">
                                {logLines.map((line) => {
                                    if (line.type === 'header') {
                                        return (
                                            <div key={line.id} className="flex items-center gap-2 text-cyan-400 font-bold border-b border-slate-800/60 pb-1 pt-4 first:pt-0">
                                    ...
                                    if (line.level === 'WRN') lineClass = "text-amber-400 bg-amber-950/30 px-1 rounded-sm";
                                    if (line.level === 'ERR') lineClass = "text-rose-400 bg-rose-950/40 px-1 rounded-sm font-semibold";

                                    return (
                                        <div key={line.id} className={`whitespace-pre-wrap break-all transition-colors hover:bg-slate-900/50 py-0.5 px-1 rounded-sm ${lineClass}`}>
                                            {line.raw}
                                        </div>
                                    );
                                })}
