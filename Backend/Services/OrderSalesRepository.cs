...
public class OrderSalesRepository : IOrderSalesRepository
{
    ...
    public async Task<IEnumerable<OperatingUnitDto>> GetOperatingUnitsAsync(CancellationToken ct = default)
    {
        ...
        await using var conn = new SqlConnection(_connectionString);
        var results = await conn.QueryAsync<OperatingUnitDto>(new CommandDefinition(sql, cancellationToken: ct));

        // Clean up the text by removing " Operating Unit"
        return results.Select(item => item with
        {
            OuName = item.OuName.Replace(" Operating Unit", "").Trim()
        });
    }
}
