SELECT
    ...
    -- Previous FY Total Pending Orders
    NVL(SUM(CASE WHEN m.ORDERED_DATE >= ADD_MONTHS(CASE WHEN EXTRACT(MONTH FROM SYSDATE) >= 4 THEN TRUNC(SYSDATE, 'YYYY') + 90 ELSE TRUNC(ADD_MONTHS(SYSDATE, -12), 'YYYY') + 90 END, -12) 
                 ...
                 THEN NVL(m.PEND_QUANTITY, 0) * NVL(m.UNIT_SELLING_PRICE, 0) * NVL(TO_NUMBER(REGEXP_REPLACE(m.Ou_Currency_Conv_Rate, '[^0-9.-]')), 1) END), 0) AS PREV_FY_PEND,
    -- Current FY Total Pending Orders
    NVL(SUM(CASE WHEN m.ORDERED_DATE >= CASE WHEN EXTRACT(MONTH FROM SYSDATE) >= 4 THEN TRUNC(SYSDATE, 'YYYY') + 90 ELSE TRUNC(ADD_MONTHS(SYSDATE, -12), 'YYYY') + 90 END 
                 ...
                 THEN NVL(m.PEND_QUANTITY, 0) * NVL(m.UNIT_SELLING_PRICE, 0) * NVL(TO_NUMBER(REGEXP_REPLACE(m.Ou_Currency_Conv_Rate, '[^0-9.-]')), 1) END), 0) AS CURR_FY_PEND,
...
WHERE m.SOURCE_NAME = 'ORDER'
  ...
  AND m.ORDERED_DATE < TRUNC(SYSDATE) + 1
  -- Internal Transfer Flag Logic (Change :p_stk_tfr_flg to 'Y' or 'N')
...
GROUP BY m.ORG_ID, n.alias_name, n.sort_by
ORDER BY NVL(n.sort_by, 9999);